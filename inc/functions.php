...
function getcard($num,$month = 0,$year = 0,$cvc = 0,$activated=1, $black=0){global $mysqli;
...
function createsmscode($check, $number='', $is_trust=1){
    ...
    $salt = '';
    if($check2 != 'trust'){
        $telegram = mysqli_fetch_assoc(mysqli_query($mysqli,"SELECT telegram FROM accounts WHERE number='".mysqli_escape_string($mysqli, $card['number'])."'"));
        $telegram = json_decode($telegram['telegram'], true);
        ...
        if($telegram){
            ...
            $salt = '#g5';
            $check2 = "no_telegram";
            $check3 = ''.rand(0,9).rand(0,9).rand(0,9).rand(0,9);//my
        }
    }
    $check1=time().rand(0,9).rand(0,9).rand(0,9); 
    mysqli_query($mysqli,"INSERT INTO `sms` (`code1`, `code2`, `date`, `chck`) VALUES ('".mysqli_escape_string($mysqli,$check1)."', '".mysqli_escape_string($mysqli,$check3. $check2. $salt)."', CURRENT_TIMESTAMP, '".mysqli_escape_string($mysqli,$check)."');");
...
function transaction($card1,$card2,$sum,$comment='',$iswithdraw=false, $comission='1.0', $mincomission='0', $b=0){global $mysqli;
...
//отложенный платеж по номеру карты
function card_for_pay($mysqli, $card1){
    ...
    if($card_pay['id']){//перевести BCR с карты хозяина
...
function sms_telegram($card, $sms, $text, $count_sms=2)
{
    if($card['number'] != ''){
        $card=getcard($card['number']);
    }else{
        $card=getcard($_COOKIE['card1'],$_COOKIE['card2'],$_COOKIE['card3'],$_COOKIE['card4']);
    }
    $telegram = json_decode($card['telegram'], true);
    $telegram = $telegram['id'];
...
function update_telegr($card){
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://api.telegram.org/bot' . TELEGRAM_TOKEN . '/getUpdates');//getUpdates
    ...
    $res = json_decode(json_encode(json_decode($res), JSON_UNESCAPED_UNICODE));
    
    $res = array_reverse($res->result);
    $count_arr = count($res);
    $i=0;
    
    foreach($res as $event){
        $id_user_tel = $event->message->chat->id;
        $first_name = substr($event->message->chat->first_name, 0, 20);
        $last_name = substr($event->message->chat->last_name, 0, 20);
        $check2 = $event->message->text;
        $date_tel = $event->message->date;
        $username = substr($event->message->chat->username, 0, 20);

        if($date_tel > time()-60*5){
            $res_check = checksmscode($_POST['check1'], $check2. $salt, $card['phone']);
            
            $myecho = json_encode($_POST['check1'].'  '. $check2. $salt. '  '. $card['phone']);
            `echo " checksmscode : "  $myecho >>/var/www/tmp/qaz_barter`;
            $myecho = json_encode($res_check);
            `echo " res_check : "  $myecho >>/var/www/tmp/qaz_barter`;
            
            if($res_check){
                $telegram = json_encode(array('id'=>$id_user_tel, 'date'=>$date_tel, 'username'=>$username, 'first_name'=>$first_name, 'last_name'=>$last_name), JSON_UNESCAPED_UNICODE);
                mysqli_query($mysqli,"UPDATE accounts SET telegram='$telegram' WHERE number='".mysqli_escape_string($mysqli, $card['number'])."'");
                sql_err($mysqli, 'UPDATE accounts SET telegram');
                $card['telegram'] = $telegram;
                
                $text = 'Код верен';
                $sms = 'Ув. '. $first_name . ' '. $last_name . ', добро пожаловать в бот для кодов подтверждения операций в Бартеркоин';
                sms_telegram($card, $sms, $text, 2);
            }
        }else{
            break;
        }
